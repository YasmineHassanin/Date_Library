    ...
    static ClsDateLibrary ConverTotalDaysToDateFromBeginningTheYear(short OrderofDays,short year)
{
  ...
  short daysUntilTheEndOfAYear()
  {
   ...
   /*  struct stPeriod
     {
         ClsDateLibrary dateFrom ;
         ClsDateLibrary dateEnd;
     };

     bool ArePeriodsOverLapping(stPeriod per1, stPeriod per2)
     {
         if ((CompareTwoDate(per2.dateEnd, per1.dateFrom) == enReturnDate::before || CompareTwoDate(per2.dateFrom, per1.dateEnd) == enReturnDate::after))
             return true;
         else
             return false;
     }
     short LengthOfPeriod(stPeriod per1, bool IncludingDay1 = false)
     {
         return (TheDifferanceBetweenTwoDates(per1.dateFrom, per1.dateEnd, IncludingDay1));
     }
     bool CheckIfDateWithinThePeriodOrNot(stPeriod per1, ClsDateLibrary date)
     {
         return !(CompareTwoDate(date, per1.dateFrom) == enReturnDate::before || CompareTwoDate(date, per1.dateEnd) == enReturnDate::after);

     }
     int CountDaysAreInOverLappingPeriods(stPeriod per1, stPeriod per2)
     {
         short OverLappingdays = 0;
         short LengthPeriod1 = LengthOfPeriod(per1, true);
         short LengthPeriod2 = LengthOfPeriod(per2, true);
         if (!ArePeriodsOverLapping(per1, per2))
             return 0;

         if (LengthPeriod1 < LengthPeriod2)
         {
             while (IsDate1BeforeDate2(per1.dateFrom, per1.dateEnd))
             {
                 if (CheckIfDateWithinThePeriodOrNot(per2, per1.dateFrom))
                     OverLappingdays++;

                 per1.dateFrom = IncreaseDateByOneDay(per1.dateFrom);
             }
         }
         else
         {
             while (IsDate1BeforeDate2(per2.dateFrom, per2.dateEnd))
             {
                 if (CheckIfDateWithinThePeriodOrNot(per1, per2.dateFrom))
                     OverLappingdays++;

                 per2.dateFrom = IncreaseDateByOneDay(per2.dateFrom);
